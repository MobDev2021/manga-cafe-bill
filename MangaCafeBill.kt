...
class MangaCafeBill(val courseName: String, val timeEntered: ZonedDateTime, val timeExited: ZonedDateTime) {
    ...
    val extensionNightFee: Int = (extensionBaseFee * nightFeePercentage).toInt()
    ...
    // 正常に動作せず★★正常に動き次第extensionFee()に改名★★
    // 延長料金計算 (22:00〜翌朝5:00は深夜料金として15%割増した金額を合計する。1分でも深夜時間に含まれていれば金額を割増、計上する。)
    fun extensionAtNight(): Int {
        // 合計延長金額
        var totalExtensionFee = 0
    ...
    // ★★extensionAtNight()が正常に動き次第削除、他のご利用時間計算関数作成★★　ご利用時間(分）を計算。
        ...
        if ((extension.toSeconds()) > 0) {
            ...
            if ((extension.toMinutesPart() % extensionBase) > 0) {
                return extension.truncatedTo(ChronoUnit.HOURS)
                    .plusMinutes(
                        extensionBase * ceil(extension.toMinutesPart().toDouble() / extensionBase)
                            .toLong()
                    )
                    .toMinutes()
            }
    ...
    // ★★extensionAtNight()が正常に動き次第削除★★
    fun extensionFee(): Int {
        return (extensionInMinutes() / extensionBase * extensionBaseFee).toInt()
    }
    ...
    fun printBill() {
        ...
        println("延長(${extensionBase}分毎)：${extensionInMinutes()}分")
...
fun main(args: Array<String>) {
    ...
    //　深夜延長動作テスト(動作させることができなかったので、深夜料金がない計算法を現在表示しております。)
    println()
    println("extensionAtNight()を動作させることができなかったので、深夜料金がない計算法を現在表示しております。")
    println(bill1.extensionAtNight())
    println(bill2.extensionAtNight())
    println(bill3.extensionAtNight())
}
